  ...
  return (
    <div className="flex flex-col gap-4">
              ...
              <div className="flex-1 space-y-1">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-950/60 border border-white/5 rounded-lg py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-slate-400/50 transition-all placeholder:text-slate-600 font-medium text-white shadow-inner"
                  />
              ...
              <div className="flex flex-col gap-1 mt-5">
                ...
                {!isLast && (
                  <button 
                    ...
                    className="p-2 text-slate-500 hover:text-blue-400 hover:bg-blue-400/10 rounded-md transition-colors"
      ...
      <div className="grid grid-cols-3 gap-2 p-1 bg-slate-950/60 rounded-lg border border-white/5">
        {[
          ...
          <button
            ...
            className={`py-3 rounded-md flex items-center justify-center transition-all duration-200 ${mode === m.id ? 'bg-slate-700 text-white shadow-lg border border-white/10 transform scale-[1.02]' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}`}
          >
            <m.icon size={18} />
          </button>
        ))}
      ...
      <button 
        ...
        className={`w-full mt-2 py-4 rounded-lg font-black flex items-center justify-center gap-2 transition-all duration-300 group relative overflow-hidden ${loading || waypoints.filter(w => w.place).length < 2 ? 'bg-slate-800/50 text-slate-600 cursor-not-allowed border border-white/5' : 'bg-slate-100 text-slate-950 hover:bg-white border border-white/10 shadow-lg active:scale-[0.98]'}`}
      >
